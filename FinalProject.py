...
class Ball(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        if self.x > width - self.rect.left:
            self.direction = (360 - self.direction) % 360
            self.x = width - self.rect.left - 1
...
def check_brick_collision(sprite1, sprite2):
    ...
    deadblocks = pygame.sprite.spritecollide(sprite1, sprite2, True)
...
def run_game():
    ...
    while not False:  # main game loop
        ...
        if pygame.sprite.collide_rect(bar, ball):
            ...
            ball.rect.y = display_surface.get_height() - bar.rect.height - ball.rect.height - 1
            ball.bounce(diff)

        check_brick_collision(ball, blocks)
